...
public class KMeans {
     ...
     * @param centroidList  Optional parameter, if given, then the list contains the beginning centroids.
     * @return K clusters along with their features.
     */
    public static Map<Centroid, List<Record>> fit(List<Record> records, int k, Distance distance, int maxIterations, Optional<List<Centroid>> centroidList) {
        applyPreconditions(records, k, distance, maxIterations);

        centroids = centroidList.orElseGet(() -> randomCentroids(records, k));
